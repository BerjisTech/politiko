...
class PoliticiansControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create politician' do
    assert_difference('Politician.count') do
      post politicians_url,
           params: { politician: { alias: @politician.alias, current_party: @politician.current_party,
                                   history: @politician.history, manifesto: @politician.manifesto, name: @politician.name } }
    end
  ...
  test 'should update politician' do
    patch politician_url(@politician),
          params: { politician: { alias: @politician.alias, current_party: @politician.current_party,
                                  history: @politician.history, manifesto: @politician.manifesto, name: @politician.name } }
